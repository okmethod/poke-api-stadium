...
export interface TypeMemoryCard {
  ...
  /** マッチ確定時に判明した共通タイプ（未確定は null） */
  readonly matchedType: PokeTypeName | null;
}

// --- ストア定義（書き込みはすべて storeWriter 経由） ---

const isLoadingStore = writable<boolean>(false);
const cardsStore = writable<TypeMemoryCard[]>([]);
...
export const storeWriter = {
  ...
  setCards: (v: TypeMemoryCard[]) => cardsStore.set(v),
  updateCard: (
    index: number,
    updates: { isFlipped?: boolean; isMatched?: boolean; matchedType?: PokeTypeName | null },
  ) => cardsStore.update((all) => all.map((c, i) => (i === index ? { ...c, ...updates } : c))),
  resetCardsState: () => {
    cardsStore.update((cs) => cs.map((c) => ({ ...c, isFlipped: false, isMatched: false, matchedType: null })));
    firstSelectedIndexStore.set(null);
    isCheckingStore.set(false);
    matchedPairCountStore.set(0);
    moveCountStore.set(0);
